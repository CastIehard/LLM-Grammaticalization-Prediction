...
DEV_SET_FILE = "dev_set.jsonl"

TAG = f"{MODEL_NAME.replace(':', '_').replace('/', '_')}_AUTOCT_ABC"
OUTPUT_DIR = "output"
...
# === Load Dev Set Examples for Auto-CoT ===
EXAMPLES_PER_LEVEL = 2
with open(DEV_SET_FILE, "r", encoding="utf-8") as f:
    dev_data = [json.loads(line) for line in f if line.strip()]

dev_examples_by_level = {i: [] for i in range(1, 5)}
for item in dev_data:
    level = int(item["gramm_score"])
    if len(dev_examples_by_level[level]) < EXAMPLES_PER_LEVEL:
        dev_examples_by_level[level].append(item)
    if all(len(v) == EXAMPLES_PER_LEVEL for v in dev_examples_by_level.values()):
        break

# === Prompt Builders (Auto-CoT) ===
def build_prompt_a_auto(sentence):
    examples = ""
    for level in sorted(dev_examples_by_level):
        for ex in dev_examples_by_level[level]:
            examples += f"""Input sentence:  
{ex["sentence_raw"]}  
Label: [{level}]
...
def build_prompt_b_auto(sentence):
    examples = ""
    for level in sorted(dev_examples_by_level):
        for ex in dev_examples_by_level[level]:
            examples += f"""Sentence:  
...
def build_prompt_c_auto(sentence):
    examples = ""
    for level in sorted(dev_examples_by_level):
        for ex in dev_examples_by_level[level]:
            examples += f"""Example:  
...
for item in tqdm(data):
    ...
    prompts = {
        "A": build_prompt_a_auto(sentence),
        "B": build_prompt_b_auto(sentence),
        "C": build_prompt_c_auto(sentence)
    }
