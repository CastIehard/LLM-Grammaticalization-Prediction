...
TAG = f"{MODEL_NAME}_ZS_ABC_WRDLEN"
OUTPUT_DIR = "output"
...
WORDLEN_DICT = {entry["keyword"]: len(entry["keyword"]) for entry in test_data}
...
def build_prompt_a(sentence, keyword):
    length_info = f"\n\nThe length of the bracketed keyword '{keyword}' (in characters) is: {WORDLEN_DICT.get(keyword, 'unknown')}."
...
In addition to the sentence, consider the length of the of the preposition as an additional signal to predict the grammaticalization.
{length_info}
...
In addition to the sentence, consider the length of the preposition as an additional signal to predict the grammaticalization.{length_info}
...
for item in tqdm(test_data):
    ...
    row = {"sentence": sentence, "keyword": keyword, "gramm_score": true, "word_length": len(keyword)}
    for k in "ABC":
        prompt = prompts[k]
